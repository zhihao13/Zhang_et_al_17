function stats_p_r=compareRefRDM2candRDMs_ZZ(refRDM, candRDMs, userOptions)

% This is Zhihao's modification on the original compareRefRDM2candRDMs
% function in Krieskorte's RSA toolbox (Nili et al. 2013)
%
% Key changes include:
% 1. When there are multiple instances of candidate model RDMs (most likely
% a result of multiple subjects), instead of averaging the candidate model
% RDMs like the original function does, this _ZZ version compares one candidate RDM
% to one corresponding neural RDM in refRDM. (Of course, this requires that
% the numbers of refRDM matrices and candidate RDMs are the same, and there
% is a one-to-one correspondence.
% 2. Removed the graph-making part of the original function and, as a
% result, also fixed the bug when none of the pairwise comparisons of candidate
% RDMs are significant
% 
% Note: This script may not be suitable for analysis options other than
% SubjectRFXsignedRank as of 3/29/2016
% 
% 
% See the original documentation of the function below:
%
% GENERAL PURPOSE
...
%       false). If set to true, bars corresponding to candidate RDMs are
...
%       : corresponding uncorrected p values
...
userOptions = setIfUnset(userOptions, 'barsOrderedByRDMCorr',false);
...
[~,nCond,nRefRDMinstances]=size(refRDM_stack);
...
% Zhihao's note: This is where I changed the original function from
% comparing the refRDM against the average candidate RDM (when NrOfSubjects
% > 12) into against individual instances of candidate RDMs
...
for candI = 1:numel(candRDMs)
    for subI = 1:nSubjects
        if isequal(userOptions.RDMcorrelationType,'Kendall_taua')
            cand2refSims(subI,candI)=rankCorr_Kendall_taua(vectorizeRDMs(candRDM_stack{candI}(:,:,subI))',vectorizeRDMs(refRDM_stack(:,:,subI))');
        elseif isequal(userOptions.RDMcorrelationType,'raeSpearman')
            cand2refSims(subI,candI)=raeSpearmanCorr(vectorizeRDMs(candRDM_stack{candI}(:,:,subI))',vectorizeRDMs(refRDM_stack(:,:,subI))');
        else
            cand2refSims(subI,candI)=corr(vectorizeRDMs(candRDM_stack{candI}(:,:,subI))',vectorizeRDMs(refRDM_stack(:,:,subI))','type',userOptions.RDMcorrelationType,'rows','pairwise');
        end
    end
end
...
%         fprintf('\nPerforming signed-rank test of RDM relatedness (subject as random effect).\n');
        ...
        % Further work will be needed on this part if we also want
        % subjectRFXbootstrap with subject-specific candidate RDMs
...
%         fprintf('\nPerforming subject bootstrap test of RDM relatedness (subject as random effect).\n');
        ...
        % Further work will be needed on this part if we also want
        % subjectConditionRFXbootstrap with subject-specific candidate RDMs     
...
%         fprintf('\nPerforming subject and condition bootstrap test of RDM relatedness (subject and condition as random effects).\n');
        ...
        % Further work will be needed on this part if we also want
        % randomisation with subject-specific candidate RDMs       
    case 'randomisation'
...
if ~isequal(userOptions.RDMrelatednessTest,'none')
    ...
    elseif isequal(userOptions.RDMrelatednessMultipleTesting,'FDR')
        thresh = FDRthreshold_ZZ(ps,userOptions.candRDMdifferencesThreshold);
    end
...
switch userOptions.candRDMdifferencesMultipleTesting
    case 'FDR'
        ...
        threshold = FDRthreshold_ZZ(allPairwisePs,thresh);
